		...
		if r1.status_code == requests.codes.ok:
			for network_path in r1.json():
				if network_path['sourceAppliance'] == row['source_mp'] and network_path['target'] == row['target']:
					print('Matching path exists: Org id = {}, Source MP = {}, Target = {} -> Network path ({}) ({})'.format(row['org_id'], network_path['sourceAppliance'], network_path['target'], network_path['id'], network_path['pathName']))
